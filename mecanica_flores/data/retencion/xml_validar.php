<?php
    ...
    $count = 0;
    if($_GET['estado'] == "0") {
        $resultado = $class->consulta("SELECT  COUNT(*) AS count from retencion");
    } else {
        $resultado = $class->consulta("SELECT  COUNT(*) AS count from retencion WHERE estado = '".$_GET['estado']."'");
    }
    ...
    if($_GET['estado'] == "0") {
        if ($search == 'false') {
            $SQL = "SELECT R.id, R.numero_autorizacion, R.fecha_emision, P.nombre_comercial, R.estado, R.fecha_autorizacion, R.clave_acceso, R.total_retenido FROM retencion R, proveedores P WHERE R.id_proveedor = P.id ORDER BY $sidx $sord limit $limit offset $start";
        } 
    } else {
        if ($search == 'false') {
            $SQL = "SELECT R.id, R.numero_autorizacion, R.fecha_emision, P.nombre_comercial, R.estado, R.fecha_autorizacion, R.clave_acceso, R.total_retenido FROM retencion R, proveedores P WHERE R.id_proveedor = P.id AND R.estado = '".$_GET['estado']."' ORDER BY $sidx $sord limit $limit offset $start";
        }    
    }
