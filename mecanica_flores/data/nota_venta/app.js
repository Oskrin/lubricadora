app.controller('nota_ventaController', function ($scope, $interval) {
	...
	jQuery(function($) {
		...
		// abrir en una nueva ventana detalle notas
		$scope.methodoview = function(id) { 
			...
			$.ajax({
				url: 'data/nota_venta/app.php',
				type: 'post',
				data: {llenar_cabezera_nota:'llenar_cabezera_nota', id: id},
				dataType: 'json',
				success: function(data) {
		...
		$scope.methodspdf = function(id) { 
			var myWindow = window.open('data/reportes/nota_venta.php?id='+id,'popup','width=900,height=650');
		} 
		// fin
		...
		$('#btn_3').click(function() {
			if($('#id_nota').val() == '') {
				$.gritter.add({
					title: 'Seleccione Nota a Reimprimir',
					class_name: 'gritter-error gritter-center',
					time: 1000,
				});	
			} else {
		...
		$('#btn_brir').click(function() {
			...
			if(filas.length == 0) {
                $.gritter.add({
					title: 'Ingrese productos a la Nota Venta',
					class_name: 'gritter-error gritter-center',
					time: 1000,
				});
                $('#codigo_barras').focus();
		...
		$('#btn_actualizar').click(function() {
			jQuery("#table5").jqGrid('setGridParam',{datatype: 'local', data:mydata ,page:1}).trigger("reloadGrid");
			//jQuery("#table5").jqGrid('setGridParam',{datatype: 'local', data:mydata, page:1}).trigger("reloadGrid");
		...
		function enviar() {
            ...
            $.ajax({
                ...
                success: function(response) {
                	...
                	var id = response;
	        		bootbox.alert("Gracias! Por su Información Nota Venta Agregada Correctamente!", function() {
					  	var myWindow = window.open('data/reportes/nota_venta.php?id='+id,'popup','width=900,height=650'); 
					  	location.reload();
					});		
                }
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector1).jqGrid({	 
	    	datatype: "xml",
		    url: 'data/nota_venta/xml_nota_venta.php',         
	        ...
	        gridComplete: function() {
				...
				for(var i = 0;i < ids.length;i++) {
					var id_nota = ids[i];
					pdf = "<a onclick=\"angular.element(this).scope().methodspdf('"+id_nota+"')\" title='Reporte Factura Venta' ><i class='fa fa-file-pdf-o red2' style='cursor:pointer; cursor: hand'> PDF</i></a>"; 
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector2).jqGrid({	 
	    	datatype: "xml",
		    url: 'data/nota_venta/xml_proforma.php', 
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector5).jqGrid({	        
	        ...
	        data: mydata,
	        //datatype: "json",
	        //url: "data/inicio/app.php?cargar_tabla2="+'cargar_tabla2',      
	        ...
	        colModel:[      
	            ...
	            {name:'disponibles',index:'disponibles',frozen : true, hidden: false, align:'left',search:false,width:'100'},
