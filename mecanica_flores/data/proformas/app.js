app.controller('proformasController', function ($scope, $interval) {
	...
	jQuery(function($) {
		...
		// tabs Proforma
		$( "#tabProforma" ).click(function(event) {
			event.preventDefault();  
		});

		$("#tabProforma").on('shown.bs.tab', function(e) {
		...
		// abrir en una nueva ventana detalle proformas
		$scope.methodoview = function(id) { 
			$('#myModalProformas').modal('show');
			$("#detalle-table tbody tr").remove(); 

			$.ajax({
				url: 'data/proformas/app.php',
				type: 'post',
				data: {llenar_cabezera_proforma:'llenar_cabezera_proforma', id: id},
				dataType: 'json',
				success: function(data) {
					$scope.id = data.id_proforma;
					...
					$scope.telefono = data.telefono2;
					$scope.email = data.correo;
					
					$scope.subtotal = data.subtotal;
					$scope.subtotal_iva = data.tarifa;
					$scope.subtotal_0 = data.tarifa0;
					$scope.iva = data.iva;
		...
		$scope.methodosend = function(id) {
		    ...
		    $.ajax({
				url: 'data/proformas/app.php',
				type: 'post',
				data: {cargar_correo:'cargar_correo', id: id},
				//dataType: 'json',
				success: function(response) {
			...
			$.ajax({
				...
				success: function(response) { 
					if(response == null) {
						...
						for (var i = 0; i < response.length; i++) {
							var estado = "<span class='badge badge-danger'>Borrador</span>";							
							var vizualizar = "<button type='button' class='btn btn-white btn-primary btn-round btn-sm' onclick=\"angular.element(this).scope().methodoview('"+response[i].id+"')\" data-toggle='tooltip' title = 'Visualizar Proforma'><i class='ace-icon fa fa-eye bigger-120 blue'></i></button>";
							var enviar = "<button type='button' class='btn btn-white btn-primary btn-round btn-sm' onclick=\"angular.element(this).scope().methodosend('"+response[i].id+"')\" data-toggle='tooltip' title = 'Enviar Proforma'><i class='ace-icon fa fa-envelope bigger-120 blue'></i></button>";
							var accion = vizualizar + enviar;
		...
		$scope.methodspdf = function(id) { 
			var myWindow = window.open('data/proformas/proforma.php?id='+id,'popup','width=900,height=650');
		} 
		// fin
	...
	jQuery(function($) {
	    ...
	    jQuery(grid_selector2).jqGrid({	 
	        ...
	        colNames: ['ID','IDENTIFICACIÓN','RAZÒN SOCIAL','SERIE','FECHA EMISIÓN','TOTAL','ESTADO','PDF','COMPARTIR'],
	        colModel:[ 
			    {name:'id',index:'id', frozen:true, align:'left', search:false, hidden: true},   
	            {name:'C.identificacion',index:'C.identificacion', frozen:true, align:'left', search:true, hidden: false, width: '120'},
	            {name:'C.nombres_completos',index:'C.nombres_completos',frozen : true,align:'left', search:true, width: '230'},
	            {name:'secuencial',index:'secuencial',frozen : true, hidden: false, align:'left', search:true,width: ''},
	            {name:'fecha_emision',index:'fecha_emision',frozen : true, align:'left', search:false,width: '120'},
	            {name:'total_pagar',index:'total_pagar',frozen : true, align:'left', search:false,width: '60'},
	            ...
	            {name:'share', index:'share', editable: false, hidden: true, frozen: true, editrules: {required: true}, align: 'center', width: '100px'}
	        ],          
	        rowNum: 10,       
	        rowList: [10,20,30],
	        width: 600,
