<?php
	...
	function generarPDF($id) {
		...
		$resultado = $class->consulta("SELECT F.id, F.fecha_emision, F.secuencial, F.id_cliente, C.identificacion, C.razon_social, C.direccion, C.telefono2, C.correo, F.estado FROM factura_venta F, clientes C WHERE F.id_cliente = C.id AND F.id = '".$id."'");
		while ($row = $class->fetch_array($resultado)) {
			$fechaEmision = $row[1];
			$secuencial = $row[2];
		...
		$pdf->SetFont('Arial','B',10);		 	
		$pdf->Text(108, 29, utf8_decode("FACTURA VENTA")); 
		$pdf->SetFont('Arial','',10);
		...
		$resultado = $class->consulta("SELECT P.codigo, P.descripcion, D.cantidad, D.precio, D.descuento, D.total FROM detalle_factura_venta D, factura_venta F, productos P WHERE D.id_factura_venta = F.id AND D.id_producto = P.id AND D.id_factura_venta = '".$id."'");
		while ($row = $class->fetch_array($resultado)) {
		...
		if($pdf->getY() <= 220) {
			...
			$resultado = $class->consulta("SELECT F.subtotal, F.tarifa, F.tarifa0, F.iva, F.total_descuento, F.total_venta, F.estado FROM factura_venta F, clientes C WHERE F.id_cliente = C.id AND F.id = '".$id."'");		
			while ($row = $class->fetch_array($resultado)) {
				$subtotal = $row[0];
				$tarifa = $row[1];
				$tarifa0 = $row[2];
