<?php
    ...
    $count = 0;
    if($_GET['estado'] == "0") {
        $resultado = $class->consulta("SELECT  COUNT(*) AS count from guia_remision");
    } else {
        $resultado = $class->consulta("SELECT  COUNT(*) AS count from guia_remision WHERE estado = '".$_GET['estado']."'");
    }
    ...
    if($_GET['estado'] == "0") {
        if ($search == 'false') {
            $SQL = "SELECT G.id, G.numero_autorizacion, G.fecha_emision, T.nombre_comercial, G.estado, G.fecha_autorizacion, G.clave_acceso FROM guia_remision G, transportistas T WHERE G.id_transportista = T.id ORDER BY $sidx $sord limit $limit offset $start";
        } 
    } else {
        if ($search == 'false') {
            $SQL = "SELECT G.id, G.numero_autorizacion, G.fecha_emision, T.nombre_comercial, G.estado, G.fecha_autorizacion, G.clave_acceso FROM guia_remision G, transportistas T WHERE G.id_transportista = T.id AND F.estado = '".$_GET['estado']."' ORDER BY $sidx $sord limit $limit offset $start";
        }    
    }
