app.controller('transportistasController', function ($scope, $route, loaddatosSRI) {
	...
	jQuery(function($) {
		...
		// tabs
		$( "#tabTransportista" ).click(function(event) {
			event.preventDefault();  
		});	

		$("#tabTransportista").on('shown.bs.tab', function(e) {
			...
			if(e.keyCode == 13) { //tecla del alt para el entrer poner 13
				...
				$('#identificacion').focus();
					...
					if($('#select_documento').val() == '2') {
						$.ajax({
			                type: "POST",
			                url: "data/transportistas/app.php",          
	        ...
	        $.ajax({
	            ...
	            success: function(data) {
	                ...
	                if (val == 1) {
	                    ...
	                    $("#identificacion").focus();
	                    $.gritter.add({
							title: 'Error... El Transportista ya se encuentra Registrado',
							class_name: 'gritter-error gritter-center',
							time: 1000,
						});
					}
		...
		//validacion formulario transportistas
		$('#form_transportistas').validate({
			errorElement: 'div',
			...
			messages: {
				...
				identificacion: {
					...
					//unique: "El Transportista ya se encuentra Registrado"
				},
				razon_social: { 	
					required: "Por favor, Indique una Razón Social",			
				},
				nombre_comercial: { 	
					required: "Por favor, Indique un Nombre Comercial",			
				},
				telefono2: {
		...
		// inicio
		$("#logo").attr("src", "data/transportistas/fotos/defaul.jpg");
		...
		$('#btn_0').click(function() {
			var respuesta = $('#form_transportistas').valid();
			
			if (respuesta == true) {
				$('#btn_0').attr('disabled', true);
				var formData = new FormData(document.getElementById("form_transportistas"));
		...
		$('#btn_descargar').click(function() {
			var archivo = "data/transportistas/plantilla/transportistas.xlsx";
			window.open(archivo);
		});
		// fin
		...
		$('#btn_excel').click(function() {
			...
			var formData = new FormData(document.getElementById("form_cargar"));
		...
		jQuery(function($) {
		    ...
		    jQuery(grid_selector).jqGrid({	        
		        datatype: "xml",
		        url: 'data/transportistas/xml_transportistas.php',        
		        colNames: ['ID','ID_TIPO_DOCUMENTO','TIPO DOCUMENTO','IDENTIFICACIÓN','RAZÓN SOCIAL','NOMBRE COMERCIAL','TELÉFONO','CELULAR','CIUDAD','DIRECCIÓN','CORREO','FOTO','OBSERVACIONES','','ESTADO'],
		        colModel:[      
		            {name:'id',index:'id', frozen:true, align:'left', search:false, hidden: true},
		            {name:'id_tipo_documento',index:'id_tipo_documento', frozen:true, align:'left', search:false, hidden: true},
		            {name:'tipo_documento',index:'tipo_documento',frozen : true, hidden: false, align:'left',search:false,width:''},
		            {name:'identificacion',index:'identificacion',frozen : true, hidden: false, align:'left',search:true,width:'120'},
		            ...
		            {name:'telefono1',index:'telefono1',frozen : true, hidden: false, align:'left',search:false,width:'110'},
		            {name:'telefono2',index:'telefono2',frozen : true, hidden: false, align:'left',search:false,width:'110'},
		        ...
		        ondblClickRow: function(rowid) {     	            	            
	            	...
	            	$('#id_transportista').val(ret.id);
	            	$("#logo").attr("src", "data/transportistas/fotos/"+ ret.foto);
		        ...
		        editurl: "data/transportistas/app.php",
		        caption: "LISTA TRANSPORTISTAS"
		    });
