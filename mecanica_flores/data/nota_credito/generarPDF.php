<?php
	...
	function generarPDF($id) {
		...
		$resultado = $class->consulta("SELECT E.ruc, N.numero_autorizacion, N.fecha_emision, N.clave_acceso, E.razon_social, E.nombre_comercial, E.direccion_matriz, E.direccion_establecimiento, E.contribuyente, E.obligacion, C.razon_Social, C.identificacion, C.direccion, C.telefono1, C.correo, N.secuencial, N.establecimiento, N.punto_emision, N.fecha_autorizacion, TD.codigo, N.ambiente, N.emision, E.imagen, TC.codigo, F.secuencial, F.fecha_emision, N.motivo FROM nota_credito N, factura_venta F, clientes C, empresa E, tipo_documento TD, tipo_comprobante TC WHERE N.id_factura_venta = F.id AND F.id_cliente = C.id AND C.id_tipo_documento = TD.id AND F.id_tipo_comprobante = TC.id AND N.id_empresa = E.id AND N.id = '".$id."'");	
		while ($row = $class->fetch_array($resultado)) {
			...
			$emision = $row[21];
			$imagen = $row[22];
			$comprobanteModifica = $row[23];
			$numeroComprobanteModifica = $row[24];
		    $fechaComprobanteModifica = $row[25];
		    $razonModifica = $row[26];
		}
		...
		$pdf->SetFont('Arial','B',12);		 	
		$pdf->Text(108, 23, utf8_decode("NOTA CRÉDITO")); // Tipo comprobante
		...
		$pdf->Rect(3, 101, 205, 44 , 'D'); // 4 INFO TRIBUTARIA	
		$pdf->SetFont('Arial','B',9);
		...
		$pdf->SetFont('Arial','B',9);
		$pdf->Text(5, 117, utf8_decode('Fecha de Emisión:')); //fecha de emision cliente
		$pdf->SetFont('Arial','',9);
		$pdf->Text(35, 117, utf8_decode($fechaEmision)); 
		$pdf->Line(5,119,200,119);
		$pdf->SetFont('Arial','B',9);		
		$pdf->Text(5, 123, utf8_decode('Comprobante que se modifica:'));
		$pdf->SetFont('Arial','',9);
		$pdf->Text(135, 123, utf8_decode($comprobanteModifica) .'           '. $establecimiento.'-'.$puntoEmision.'-'.$numeroComprobanteModifica);
		$pdf->SetFont('Arial','B',9);
		$pdf->Text(5, 129, utf8_decode('Fecha Emisión (Comprobante a Modificar): '));
		$pdf->SetFont('Arial','',9);
		$pdf->Text(135, 129, utf8_decode($fechaComprobanteModifica));
		$pdf->SetFont('Arial','B',9);
		$pdf->Text(5, 135, utf8_decode('Razón de Modificación: '));
		$pdf->SetFont('Arial','',9);
		$pdf->SetY(132);
		$pdf->SetX(135);
		$pdf->multiCell(70, 6, substr($razonModifica,0,56), 0);
		...
		$resultado = $class->consulta("SELECT P.codigo, P.descripcion, D.cantidad, D.precio, D.descuento, D.total FROM nota_credito N, detalle_nota_credito D, productos P WHERE D.id_nota_credito = N.id AND D.id_producto = P.id  AND N.id = '".$id."'");
		while ($row = $class->fetch_array($resultado)) {
		...
		if($pdf->getY() <= 220) {
			...
			/////// Totales ////
			$resultado = $class->consulta("SELECT N.subtotal, N.tarifa, N.tarifa0, N.iva, N.total_descuento, N.total_nota FROM nota_credito N WHERE N.id = '".$id."'");
			while ($row = $class->fetch_array($resultado)) {
				$subtotal = $row[0];
				$tarifa = $row[1];
				$tarifa0 = $row[2];
